...
contract RumAccount {
    ...
    event Bind(
        address user,
        string indexed payment_provider,
        string payment_account,
        string meta,
        string memo
    );
    ...
    function unBind(address user, string memory payment_provider)
        public
        isManager
    {
        delete userAddress[payment_provider][
            account[user][payment_provider].payment_account
        ];
        delete account[user][payment_provider];

        if (_accounts[user].has[payment_provider]) {
            for (uint256 i = 0; i < _accounts[user].values.length; i++) {
                if (
                    keccak256(abi.encodePacked(_accounts[user].values[i])) ==
                    keccak256(abi.encodePacked(payment_provider))
                ) {
                    _accounts[user].values[i] = _accounts[user].values[
                        _accounts[user].values.length - 1
                    ];
        ...
        if (_providerUsers[payment_provider].has[user]) {
            for (
                uint256 i = 0;
                i < _providerUsers[payment_provider].values.length;
                i++
            ) {
                if (
                    keccak256(
                        abi.encodePacked(
                            _providerUsers[payment_provider].values[i]
                        )
                    ) == keccak256(abi.encodePacked(user))
                ) {
                    _providerUsers[payment_provider].values[i] = _providerUsers[
                        payment_provider
                    ].values[
                            _providerUsers[payment_provider].values.length - 1
                        ];
    ...
    function selfUnBind(string memory payment_provider) public {
        delete userAddress[payment_provider][
            account[tx.origin][payment_provider].payment_account
        ];
        delete account[tx.origin][payment_provider];

        if (_accounts[tx.origin].has[payment_provider]) {
            for (uint256 i = 0; i < _accounts[tx.origin].values.length; i++) {
                if (
                    keccak256(
                        abi.encodePacked(_accounts[tx.origin].values[i])
                    ) == keccak256(abi.encodePacked(payment_provider))
                ) {
                    _accounts[tx.origin].values[i] = _accounts[tx.origin]
                        .values[_accounts[tx.origin].values.length - 1];
    ...
    function providerUsersCount(string memory payment_provider)
        public
        view
        returns (uint256)
    {
