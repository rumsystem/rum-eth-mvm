...
contract PaidGroup {
  ...
  function pay(uint128 groupId) payable public noReentrancy {
    ...
    require(IERC20(item.tokenAddr).allowance(user, address(this)) >= item.amount, "please approve token allowance");
    IERC20(item.tokenAddr).transferFrom(user, address(this), item.amount);
    ...
    IERC20(item.tokenAddr).transfer(item.owner, ownerAmount);
    IERC20(item.tokenAddr).transfer(dappInfo.receiver, item.amount - ownerAmount);
